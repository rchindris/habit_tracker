...
def sort_by_name(habits: List[Habit]) -> List[Habit]:
...
def _get_period_dates(
        current_date: date, periodicity: Periodicity, offset: int = 0
) -> Tuple[date, date]:
...
def get_broken_habits(
        habits: List[Habit], as_of_date: date = None
) -> List[Tuple[str, date, Periodicity]]:
